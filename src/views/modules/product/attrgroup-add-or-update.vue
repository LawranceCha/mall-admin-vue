  ...
  <el-dialog :title="!dataForm.attrGroupId ? '新增' : '修改'"
    ...
    <el-form :model="dataForm"
      ...
      <el-form-item label="所属分类id"
                    prop="catelogId">
        <!-- <el-input v-model="dataForm.catelogId"
                  placeholder="所属分类id"></el-input> 
                     @change="onCacaderChange"-->
  ...
  methods: {
    getCategorys () {
      api.getCategoryTree().then(({ data }) => {
        ...
        this.categorys = data.listWithTree || [];
      });
    },
    init (id) {
      ...
      this.$nextTick(() => {
        ...
        if (this.dataForm.attrGroupId) {
          this.$http({
            url: this.$http.adornUrl(`/mall-product/attrgroup/info/${this.dataForm.attrGroupId}`),
            method: 'get',
    ...
    dataFormSubmit () {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          this.$http({
            url: this.$http.adornUrl(`/mall-product/attrgroup/${!this.dataForm.attrGroupId ? 'save' : 'update'}`),
            method: 'post',
